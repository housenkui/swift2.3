...
import UIKit
public protocol ChinessSubDivPickerDeleagte{
    func subDivPickerDidUpdate(sender:ChinessSubDivPicker)
}
public class ChinessSubDivPicker: UIPickerView ,UIPickerViewDelegate,UIPickerViewDataSource{
    ...
    private lazy var subDivisionsData:[Province] = {
        ...
        guard let path = podBundle.pathForResource("ChineseSubdivisions",ofType:"plist"),
            localData  = NSArray(contentsOfFile:path) as? [[String:[[String:[String]]]]] else{
                ...
                #if DEBUG
                    assertionFailure("ChinsesSubvisionsPicker load data failed")
                #endif
                return []
        }
    ...
    public var pickerType:ChineseSubDivPickerType = .District {
        didSet{
            province = nil
            city  = nil
            district = nil
    ...
    //遵守协议的属性
    public var pickerDelegate:ChinessSubDivPickerDeleagte?
    ...
    override public func didMoveToWindow() {
        ...
        pickerDelegate?.subDivPickerDidUpdate(self)
    }
    ...
    override public func selectRow(row: Int, inComponent component: Int, animated: Bool) // scrolls the specified row to center.
    {
